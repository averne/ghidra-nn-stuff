...
from lib.demangling import get_demangled
...
for i in sys.argv[1:]:
	...
	name = i
	out_name = os.path.join(os.path.dirname(name), os.path.basename(name) + '-ipc-syms.txt')
...
#				print hex(int(i.op_str[1:],16)), '%s::Process_Cmd%d' % (demangled_interface_name, current_cmd)
				pfuncname = 'nn::sf::cmif::server::detail::CmifProcessFunctionTableGetterImpl<%s>::Process_%s' % (str(demangled_interface_name), str(cmd_name))
